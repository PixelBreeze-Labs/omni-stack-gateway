// src/controllers/field-task.controller.ts (Updated with User ID passing)
import { 
  Controller, 
  Get, 
  Post,
...
export class FieldTaskController {
  ...
  // ============================================================================
  // TASK CRUD ENDPOINTS WITH USER ID EXTRACTION
  // ============================================================================
  ...
  async createTask(
    ...
    try {
      ...
      const result = await this.fieldTaskService.createTask(
        {
          businessId,
          appClientId: business.clientId, // Use business client ID
          ...taskData
        },
  ...
  async getTasks(
    ...
    try {
      ...
      // 🎯 PASS USER ID TO SERVICE
      const result = await this.fieldTaskService.getTasks(
        businessId, 
        filters,
        adminUserId, // Pass admin user ID for activity tracking
        req // Pass request for IP/UserAgent
  ...
  async updateTask(
    ...
    try {
      ...
      // 🎯 PASS USER ID TO SERVICE
      const result = await this.fieldTaskService.updateTask(
        businessId, 
        taskId, 
        updateData,
        adminUserId, // Pass admin user ID for activity tracking
  ...
  async deleteTask(
    ...
    try {
      ...
      // 🎯 PASS USER ID TO SERVICE
      const result = await this.fieldTaskService.deleteTask(
        businessId, 
        taskId,
        adminUserId, // Pass admin user ID for activity tracking
        req // Pass request for IP/UserAgent
  ...
  // ============================================================================
  // TASK ASSIGNMENT ENDPOINTS WITH USER ID
  // ============================================================================
  ...
  // ============================================================================
  // TASK STATISTICS ENDPOINT WITH USER ID
  // ============================================================================

  @Get('statistics')
  @ApiOperation({ 
    summary: 'Get task statistics',
    description: 'Retrieve task analytics and performance metrics'
  })
  @ApiQuery({ name: 'businessId', required: true, description: 'Business ID' })
  @ApiQuery({ name: 'timeframe', required: false, description: 'Timeframe for statistics (e.g., 30d, 90d)', default: '30d' })
  @ApiResponse({ 
    status: 200, 
  ...
  async getTaskStatistics(
    ...
    try {
      ...
      // 🎯 PASS USER ID TO SERVICE
      const result = await this.fieldTaskService.getTaskStatistics(
        businessId,
        timeframe,
        adminUserId, // Pass admin user ID for activity tracking
        req // Pass request for IP/UserAgent
