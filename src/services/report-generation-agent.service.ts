...
export class ReportGenerationAgentService {
  ...
  private initializeEmailTransporter() {
    // This would be configured with your actual email service
    ...
    for (const template of templates) {
      ...
      if (cronExpression) {
        ...
        const job = new CronJob(cronExpression, () => {
          this.generateReportFromTemplate(template._id);
        });
  ...
  private async sendReportEmail(
    ...
    // This would send an email with the report attached
    // or a link to download the report
    
    const attachments = [];
    
    if (template.includeAttachment && report.filePath) {
      attachments.push({
        filename: `${template.name}.${template.format.toLowerCase()}`,
    ...
    const mailOptions = {
      ...
      subject: `${template.name} - ${format(report.startDate, 'MMM d, yyyy')} to ${format(report.endDate, 'MMM d, yyyy')}`,
      html: `
        <h1>${template.name}</h1>
        <p>Please find attached the ${template.name} report for the period ${format(report.startDate, 'MMM d, yyyy')} to ${format(report.endDate, 'MMM d, yyyy')}.</p>
        ${!template.includeAttachment ? `<p>You can download the report <a href="${process.env.APP_URL}${report.fileUrl}">here</a>.</p>` : ''}
        <p>This report was automatically generated by the Report Generation Agent.</p>
      `,
      attachments
    };
